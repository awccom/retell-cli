 ...
 */
function saveRetellLlmPrompts(agentDir: string, promptSource: Extract<Awaited<ReturnType<typeof resolvePromptSource>>, { type: 'retell-llm' }>): void {
  ...
  writeFileSync(join(agentDir, 'general_prompt.md'), prompts.general_prompt || '');
 ...
 */
function saveConversationFlowPrompts(agentDir: string, promptSource: Extract<Awaited<ReturnType<typeof resolvePromptSource>>, { type: 'conversation-flow' }>): void {
  ...
  writeFileSync(join(agentDir, 'global_prompt.md'), prompts.global_prompt || '');

  // Save nodes as JSON
  writeFileSync(join(agentDir, 'nodes.json'), JSON.stringify(prompts.nodes, null, 2));
}

/**
 * Get list of files created for success message
 */
function getFilesCreated(type: string, promptSource: Awaited<ReturnType<typeof resolvePromptSource>>): string[] {
