...
import type { CreateKnowledgeBaseOptions, TextEntry, KnowledgeBaseMutationOutput } from '../../types/kb';
 ...
 */
export async function createKnowledgeBaseCommand(options: CreateKnowledgeBaseOptions): Promise<void> {
  try {
    ...
    if (options.texts) {
      ...
      try {
        ...
        const texts: TextEntry[] = textsData.map((entry: unknown, index: number) => {
