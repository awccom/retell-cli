 ...
 */
export async function exportToolsCommand(agentId: string, options: ExportToolsOptions): Promise<void> {
  try {
    ...
    if (source.type === 'retell-llm') {
      ...
      exportData = {
        ...
        tools: {
          general: source.generalTools.length > 0 ? source.generalTools : undefined,
          states: Object.keys(stateToolsExport).length > 0 ? stateToolsExport : undefined,
        },
      ...
      exportData = {
        ...
        tools: {
          ...
          components: Object.keys(componentToolsExport).length > 0 ? componentToolsExport : undefined,
        },
        total_count: source.totalCount,
      };
    }
    ...
    if (options.output) {
      try {
        writeFileSync(options.output, JSON.stringify(exportData, null, 2), 'utf-8');
        outputJson({
